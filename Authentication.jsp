...
<%
     try
	{
			...
			if(value.equalsIgnoreCase("ecommercelogin"))
			{
					String username=request.getParameter("username"); 
         			String password=request.getParameter("password");
					String ecom=request.getParameter("ECommerce");
					
					String sql = "SELECT * FROM ecommerce where username='"+username+"' and password='"+password+"' and ename='"+ecom+"' ";
					Statement stmt = connection.createStatement();
					ResultSet rs = stmt.executeQuery(sql);
					if (rs.next()==true) 
					{
		
								String sql1="SELECT * FROM ecommerce where username='"+username+"' and status='Authorized' ";
								Statement stmt1 = connection.createStatement();
								ResultSet rs1 =stmt1.executeQuery(sql1);
								if(rs1.next()==true)
								{
									...
									String eweb=rs1.getString(12);
									
									application.setAttribute("ename", username);
									application.setAttribute("eweb", eweb);
								...
								else
								{
									...
									<p align="center" class="style3">You are not Authorized by the Admin, Please wait!! </p>
									<p align="center" class="style3"><br/>
								      <a href="E_Login.jsp" class="style4">Back</a></p>
									<p>
									  <%
								 }
